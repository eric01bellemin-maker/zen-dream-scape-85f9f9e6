      ...
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6 flex-1">
          {navLinks.slice(0, 3).map((link) => (
            <a key={link.name} href={link.href} className="text-[11px] font-bold text-slate-500 hover:text-[#26A69A] tracking-wider uppercase transition-colors">
              {link.name}
            </a>
          ))}
        </div>
        ...
        <div className="hidden md:flex items-center justify-end gap-6 flex-1">
          ...
          <a href="#details" className="ml-4 flex items-center gap-2 bg-[#26A69A] text-white px-4 py-2 rounded-full text-[10px] font-bold hover:bg-[#1f8a80] shadow-md transition-all">
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 absolute w-full shadow-lg">
          <div className="flex flex-col p-4 gap-4">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} onClick={() => setIsOpen(false)} className="text-sm font-bold text-slate-600 hover:text-[#26A69A] uppercase tracking-widest">
                {link.name}
              </a>
            ))}
          </div>
